...
async def handle_message(message, bot, menu_timer):
    ...
    try:
        ...
        #         if clicks:
        #             # Get user name and display info
        #             if not is_other_user:
        #                 user_name = message.author.display_name if message.author.display_name else message.author.name
        #             else:
        #                 target_user = await bot.fetch_user(target_user_id)
        ...
        #             # Get click times for detailed history
        #             click_times = [(timer_value, click_time, get_color_emoji(timer_value, game_session['timer_duration'])) 
        #                         for timer_value, click_time, _, _ in clicks]
                    
        #             # Create and send main summary embed
        #             main_embed = nextcord.Embed(
        #                 title=f'📊 Heroic Journey of {user_name} 📊', 
        #                 color=nextcord.Color.from_rgb(106, 76, 147)
        #             )
                    
        #             main_embed.description = f"Rank: **#{rank}** out of {total_players} adventurers\n"
        #             main_embed.description += f"Total Clicks: **{len(clicks)}**\n"
        #             main_embed.description += f"Time Claimed: **{format_time(total_claimed_time)}**"
                    
        #             # Color summary
        #             color_summary = ', '.join(f'{emoji} x{count}' for emoji, count in color_counts.items())
        #             main_embed.add_field(name='🎨 Color Summary', value=color_summary, inline=False)
                    
        #             # Set footer with explanation
        #             footer_text = "Total time claimed represents the amount of time "
        #             footer_text += "you've" if not is_other_user else "they've"
        #             footer_text += " prevented the clock from reaching zero."
        ...
        #             # Now create embeds for the click history, split into chunks
        #             if click_times:
        #                 # Maximum number of clicks to show per embed
        #                 clicks_per_embed = 25
                        
        #                 # Split click_times into chunks
        #                 for i in range(0, len(click_times), clicks_per_embed):
        #                     chunk = click_times[i:i+clicks_per_embed]
                            
        #                     # Create embed for this chunk
        #                     history_embed = nextcord.Embed(
        #                         title=f'Click History ({i+1}-{min(i+clicks_per_embed, len(click_times))} of {len(click_times)})',
        #                         color=main_embed.color
        #                     )
                            
        #                     # Format the click history with timestamps and colors
        #                     history_text = ""
        #                     for idx, (timer_value, click_time, emoji) in enumerate(chunk, start=i+1):
        #                         formatted_time = format_time(timer_value)
        #                         timestamp = click_time.strftime("%Y-%m-%d %H:%M:%S")
        #                         history_text += f"**{idx}.** {emoji} `{formatted_time}` - <t:{int(click_time.timestamp())}:R>\n"
                            
        #                     history_embed.description = history_text
                            
        #                     # Add emoji sequence for this chunk (compact view)
        #                     emoji_sequence = ' '.join([emoji for _, _, emoji in chunk])
        #                     if emoji_sequence:
        #                         history_embed.add_field(
        #                             name='Emoji Sequence', 
        #                             value=emoji_sequence if len(emoji_sequence) <= 1024 else f"{emoji_sequence[:1000]}... (truncated)",
        #                             inline=False
        #                         )
