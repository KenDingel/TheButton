...
# Local imports
from database.database import get_game_session_by_guild_id, create_game_session, get_game_session_by_id, get_all_game_channels, execute_query, game_sessions_dict, update_local_game_sessions
    ...
    try:
        ...
        if message.content.lower() == 'startbutton' or message.content.lower() == 'sb':
            ...
            try:
                ...
                if not success: logger.error(f'Error retrieving user rank: {e}, {tb}'); return
                ...
                if clicks:
                    ...
                    if not is_other_user: await message.channel.send('Alas, noble warrior, your journey has yet to begin. Step forth and make your mark upon the button!')
                    else: await message.channel.send('Alas, noble warrior, their journey has yet to begin. Step forth and make your mark upon the button!')

            except Exception as e:
                tb = traceback.format_exc()
                print(f'Error retrieving user rank: {e}, {tb}')
                logger.error(f'Error retrieving user rank: {e}, {tb}')
                if not is_other_user: await message.channel.send('An error occurred while retrieving your rank. The button spirits are displeased!')
                else: await message.channel.send('An error occurred while retrieving the user rank. The button spirits are displeased!')
                    
        elif message.content.lower() in ['leaderboard', 'scores', 'scoreboard', 'top']:
            num_entries = 5
            if len(message.content.split()) > 1:
                try: num_entries = int(message.content.split(" ")[1])
            ...
            try:
                query = '''
                    ...
                    LIMIT %s
                '''
                with lock: success = execute_query(query, (num_entries,))
                most_clicks = success
            ...
            try:
                query = '''
                    ...
                    ORDER BY lowest_click_time
                '''
                with lock: success = execute_query(query, ())
