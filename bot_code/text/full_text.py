...
from utils.utils import format_time, get_color_name

def generate_explaination_text(reset_time: int) -> str:
    """
    Generate explanation text for The Button Game with dynamic time ranges for each color.
    
    Args:
        reset_time (int): The reset time in seconds for the countdown timer
        
    Returns:
        str: Formatted explanation text with color ranges and time values
    """
    # Calculate time ranges for each color (6 equal segments)
    segment_size = reset_time / 6
    color_ranges = []
    
    for i in range(6):
        start_time = int(segment_size * i)
        end_time = int(segment_size * (i + 1))
        if i == 0:  # First range (red)
            time_range = f"0-{format_time(end_time)}"
        else:
            time_range = f"{format_time(start_time)}-{format_time(end_time)}"
        color_ranges.append(time_range)
    
    # Reverse the ranges since we display Purple->Red
    color_ranges.reverse()
    
    reset_time_formatted = format_time(reset_time)
    
    return (
        f"🚨 THE BUTTON! 🚨\n"
        f"Prepare yourselves for **The Button Game**\n"
        f"A mysterious {reset_time_formatted} countdown has begun, and the fate of the world rests in your hands! ⏰\n"
        f"Keep the button alive and prevent the timer from reaching zero!\n"
        f"Each adventurer must wait between clicks to regain their strength. Choose wisely!\n\n"
        f"The button color represents the remaining time:\n"
        f"- 🔴 Red: {color_ranges[5]} (Danger Zone!)\n"
        f"- 🟠 Orange: {color_ranges[4]} (Getting Risky!)\n"
        f"- 🟡 Yellow: {color_ranges[3]} (Caution Advised!)\n"
        f"- 🟢 Green: {color_ranges[2]} (Safe... For Now!)\n"
        f"- 🔵 Blue: {color_ranges[1]} (Steady Progress!)\n"
        f"- 🟣 Purple: {color_ranges[0]} (Nicely Done!)\n\n"
        f"As a reward for your bravery, you'll earn the colored circle of the time you clicked!\n"
        f"Use `myrank` to check your personal stats and `leaderboard` to see how you stack up against other fearless clickers!\n"
        f"Use 'check' to see if you have a click ready for action!\n\n"
        f"Do you have what it takes to keep the button alive and prevent the timer from reaching zero?\n\n"
        f"Live Game Stats: https://thebuttongame.click/\n\n"
        f"*Based on the Reddit April Fools event in 2015. https://en.wikipedia.org/wiki/The_Button_(Reddit)*"
    )
...
LORE_TEXT = """
    ...
    ***Will you join the ranks of these valiant heroes in wizard K3N's virtual tavern and lend your click to the cause?*** ⚔️ **The Button awaits, and the fate of the world hangs in the balance...** 🌍
"""

LORE_DRUNK_TEXT = """
    🧙‍♂️ *Gether round, young ones, and let old Wizzerd K3N spin you a tail of MAGIC and... and... 🌟 WONDER! Yeah, that's it!* 🍆
    ...
    *FEAR NOT*, however! There are *BRAVE HEROES* 🦸‍♀️🦸‍♂️ who have taken up the *SACRED DUTY* of pressing the button! They gather in a *MAGICAL TAVERN* 🍻, owned by the *ILLUSTRIOUS AVATAR CRAFTER*, wizard K3N! 🎨 He makes the BEST cocktails! 🍹😋
...
LORE_DRUNK_ERROR_TEXT = '🍄 **Oh dear, Wizzerd K3N had too much of the... the... *ENCHANTED ELIXIR!*** 🍾💊 *The drunk wizard lore is temporarily lost in the astral plane!* 🌀🧻'

LORE_BLACKOUT_DRUNK_TEXT = """
    🧙‍♂️ *Gether 🦄 round, yung ones, and let old 🌮 Wizzerd K3N 🍕 spin you a tail �lobster of 🎉 TRAGIC and... and... 🌟 BLUNDER! Yeah, 🛴 that's it!* 🍆
    ...
    *FEAR 🌽*, however! There are *BRAVE 🍔* 🦸‍♀️🦸‍♂️ who have taken up the *SACRED 🚽* of pressing the 🛎️! They gather in a *MAGICAL 🍺*, owned by the *ILLUSTRIOUS 🎢 CRATER*, wizard K3N! 🎨 He makes the BEST 🍹😋
...
LORE_BLACKOUT_ERROR_TEXT = '🍄 **Oh dear, Wizzerd K3N had too much of the... the... *ENCHANTED ELIXIR!*** 🍾💊 *The blackout drunk wizard lore is temporarily lost in the astral plane!* 🌀🧻 hfjksdhfdshf 🍔 jfhdsajfhkjads 🍕'