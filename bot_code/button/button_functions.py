...
#     current_seconds = timer_value
#     seconds_at_next_threshold = (next_threshold / 100) * timer_duration
#     seconds_to_next = current_seconds - seconds_at_next_threshold
#     # Bug fix: removed the subtraction of 7200 seconds which was causing the timer to count up
#     # seconds_to_next -= 7200
#     # Ensure we return a non-negative value
#     return max(0, seconds_to_next), next_color
...
class MenuTimer(nextcord.ui.View):
    ...
    #     """Update all active games with sharding and rate limiting"""
    #     if not self.active_game_ids:
    #         return
            
    #     try:
    #         # Process games in smaller batches with delays
    #         games_to_update = self.active_game_ids.copy()
    #         batch_size = 3
    ...
    #                     update_tasks.append(self.update_single_game(game_id))
                        
    #             if update_tasks:
    #                 await asyncio.gather(*update_tasks, return_exceptions=True)
                    
    #             # Stagger batches to avoid rate limits
    #             if i + batch_size < len(games_to_update):
    #                 await asyncio.sleep(0.25)  # 1 second between batches
    ...
    # REMOVING 7/10/2025 for single per game loop
    # async def update_single_game(self, game_id):
    #     try:
    #         game_session = await get_game_session_by_id(game_id)
    #         if not game_session:
    ...
    #         # await button_message.add_reaction('⚠️')
    #         # Update the timer for the button game
    #         try:
    #             game_id = str(game_id)
    #             cache_data = game_cache.get_game_cache(game_id)
    ...
    #         logger.error(f'Error processing game {game_id}: {e}\n{tb}')
    #     finally:
    #         # Remove the all reactions from the message ALL REACTIONS from itself (the bot)
    #         try:
    #             # Get message again to ensure it's still valid, get the author user (itself)
    #             # and remove all reactions from the message from the bot
    #             # This is a workaround, since we dont have the access to run clear_reactions.
    #             message = await button_message.channel.fetch_message(button_message.id)
    #             bot_author_user = self.bot.user
    #             # Remove all reactions from the bot user specifically
    #             await message.remove_reaction("⌛", bot_author_user)
    #             await message.remove_reaction("⏳", bot_author_user)
    #             await message.remove_reaction("⚠️", bot_author_user)
    #         except Exception as e:
