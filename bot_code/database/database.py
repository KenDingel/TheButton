...
# Removed to improve performance of boot 5-11-2025
# def game_sessions_dict(game_sessions_arg=None):
#     """
#     Convert game sessions into a dictionary format with proper type handling.
    
#     Args:
#         game_sessions_arg (list, optional): List of game sessions to convert
        
#     Returns:
#         dict: Dictionary of game sessions with game_id as key
#     """
#     global game_sessions, game_sessions_as_dict
#     output = {}
    
#     # Use provided sessions or global sessions
#     if game_sessions_arg is not None:
#         sessions_to_process = game_sessions_arg
#     else:
#         # Update sessions if needed
#         sessions_to_process = update_local_game_sessions()
...
#     logger.info(f'Creating game sessions dict, lengths - game_sessions: {len(sessions_to_process)}, game_sessions_as_dict: {len(game_sessions_as_dict.keys()) if game_sessions_as_dict else 0}')
    
#     for game in sessions_to_process:
#         # Skip ended game sessions
#         if game[6] is not None:
#             continue
            
#         # Convert all numeric fields to integers and ensure proper types
#         try:
#             game_id = str(game[0])  # Convert to string for consistent key type
#             formatted_output = {
#                 "game_id": int(game[0]),
...
#             }
#             # Store with both string and integer keys for backward compatibility
...
#     """Get game session by guild ID."""
#     global game_sessions
#     logger.info(f'Getting game session by guild ID: {guild_id} from {len(game_sessions)} sessions; {game_sessions[:5]}')
